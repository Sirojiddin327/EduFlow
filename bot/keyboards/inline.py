...
def debtors_group_buttons(groups):
    ...
    rows.append([InlineKeyboardButton(text="Orqaga", callback_data="back_menu")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def date_buttons():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="Boshqa sana", callback_data="att_date:other"),
            ],
            [
...
def marking_buttons(marks):
    ...
    for student_id, mark in marks.items():
        ...
        name = mark.get("name", "")
        rows.append(
            [InlineKeyboardButton(text=name, callback_data=f"att_toggle:{student_id}")]
        )
    rows.append(
...
def debtors_pagination(total_pages, group_id, page):
    ...
    if page > 1:
        nav.append(
            InlineKeyboardButton(text="Avvalgi", callback_data=f"debtors:{group_id}:{page-1}")
        )
    nav.append(InlineKeyboardButton(text=f"{page}/{total_pages}", callback_data="noop"))
    if page < total_pages:
        nav.append(
            InlineKeyboardButton(text="Keyingi", callback_data=f"debtors:{group_id}:{page+1}")
        )
    rows.append(nav)
    rows.append(
